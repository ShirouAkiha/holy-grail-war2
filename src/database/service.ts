 ...
 * have rich, authentic competition from day one.
 */
function seedDefaultMastersIfEmpty() {
  ...
  for (const s of defaultMastersSeed) {
    if (!masterStore.has(s.discordId)) {
      ...
      masterStore.set(s.discordId, profile);
    }
  }
...
/**
 * Returns a human-friendly display name for a Discord Server / Guild ID.
 */
...
/**
 * Calculates total battle wins for a Master across all PvP, duel, and war combat modes.
 */
export function calculateTotalBattleWins(master: MasterProfile): number {
  return (master.duelsWon || 0) + (master.servantKills || 0);
}

/**
 * Computes Server Masters Ranking (Guild scope) and Global Masters Ranking (Universal scope)
 * sorted by Total Grail War Wins or Total Battle Wins (all modes).
 */
export function getMasterRankings(options: {
  ...
  if (scope === 'server') {
    scopedMasters = allMasters.filter(m => {
      if (!m.guildId && !m.guildIds) return true; // Default shared local participant
    ...
    // Fallback if local list is too small
