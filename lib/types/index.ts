// Core types for Holy Grail War / Gacha RPG Discord Bot
...
export interface ServantSkill {
  ...
  value: number; // e.g. 30 for 30% or flat values
...
export interface NoblePhantasm {
  ...
  description: string;
  target: 'single' | 'aoe';
  multiplier: number; // Base damage multiplier (e.g. 600% for Buster single target)
...
export interface CraftEssence {
  ...
  hpBonus: number;
  effectText: string;
  passiveType: 'starting_np' | 'buster_up' | 'arts_up' | 'quick_up' | 'crit_dmg' | 'hp_regen';
  passiveValue: number;
  artworkUrl?: string;
}
...
export interface ServantTemplate {
  ...
  commandDeck: [CardType, CardType, CardType, CardType, CardType]; // 5 card deck
  skills: ServantSkill[];
...
export interface MasterServantInstance {
  ...
  level: number;
  experience: number;
  allocatedStats: ServantStats; // Stat points added by Master
...
export interface MasterProfile {
  ...
  commandSeals: number; // Max 3, regenerates 1 per day
  actionPoints: number; // Max 100 for Holy Grail War
  maxActionPoints: number;
  pityCount: number; // 4-star and 5-star pity tracking
  grailWarWins: number;
...
export interface GachaBanner {
  ...
  costPerPull: number; // Saint Quartz (e.g. 3)
  costTenPull: number; // Saint Quartz (e.g. 30)
  bannerArtUrl: string;
  rates: {
    ssrServant: number; // e.g. 1.0%
    srServant: number;  // e.g. 3.0%
    rServant: number;   // e.g. 40.0%
    ssrCe: number;      // e.g. 4.0%
    srCe: number;       // e.g. 12.0%
    rCe: number;        // e.g. 40.0%
  };
}
...
export interface GachaResultItem {
  ...
  rarity: Rarity;
  item: ServantTemplate | CraftEssence;
  isNew: boolean;
  isRateUp: boolean;
}
...
export interface ActiveCombatant {
  ...
  npGauge: number; // 0 to 300%
  activeBuffs: Array<{
    name: string;
    type: 'buff_atk' | 'buff_def' | 'crit_rate' | 'evade' | 'invincible' | 'stun';
    value: number;
...
export interface TurnActionChoice {
  combatantId: string;
  selectedCards: CardType[]; // 3 cards chosen for the turn
  useSkillIndex?: number; // 0, 1, 2
