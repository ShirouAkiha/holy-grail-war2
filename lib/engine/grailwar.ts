...
export function executeWarAction(
  ...
  if (action === 'set_ward' && targetParam) {
    ...
    else if (val === 'ward') desc = `established a Mage Sanctuary Bounded Field in \`${(actor as any).sanctuaryChannelName || '#general'}\` (blocks 60% incoming ambush damage & is the sole method of HP auto-regeneration)`;
...
export function setWorkshopWardInWar(
  ...
  const targetChannel = channelName || (actor as any).sanctuaryChannelName || '#general';
  if (wardType === 'ward') {
    (actor as any).sanctuaryChannelName = targetChannel;
  }
