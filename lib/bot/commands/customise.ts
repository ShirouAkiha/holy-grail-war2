...
export const customiseCommandCode = [
  ...
  "      ...uniqueCes.slice(0, 24).map(({ ce, count }) => ({ label: count > 1 ? `${ce.name} (x${count})` : ce.name, description: '★' + ce.rarity + ' ' + ce.effectText.slice(0, 40), value: ce.id }))",
  "    ];",
