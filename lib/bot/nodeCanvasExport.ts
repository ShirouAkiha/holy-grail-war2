...
export async function renderServantProfileCard(
  ...
  ctx.fillText(servant.template.servantClass.toUpperCase(), 140, 335);

  ctx.fillStyle = '#fbbf24';
  ctx.font = '18px sans-serif';
  ctx.fillText('★'.repeat(servant.template.rarity), 140, 355);
  ...
  const maxHp = Math.round(servant.template.baseHp * (1 + (servant.level - 1) * 0.05) + totalEnd * 150);
  const rawAtk = Math.round(servant.template.baseAtk * (1 + (servant.level - 1) * 0.05) + totalStr * 80);
